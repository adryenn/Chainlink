...
type Estimator interface {
	...
	Start() error
	Close() error
	EstimateGas(calldata []byte, gasLimit uint64) (gasPrice *big.Int, chainSpecificGasLimit uint64, err error)
	BumpGas(originalGasPrice *big.Int, gasLimit uint64) (bumpedGasPrice *big.Int, chainSpecificGasLimit uint64, err error)
}

// TODO: Save gas limit on the attempt and rename eth_txes.gas_limit to eth_txes.minimum_gas_limit
