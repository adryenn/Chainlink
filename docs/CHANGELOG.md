...
A new env var has been added `GAS_ESTIMATOR_MODE`. This supercedes and replaces `GAS_UPDATER_ENABLED`. Valid values are:
...
New gas estimator modes may be added in future. Node operators are encouraged to switch from `GAS_UPDATER_ENABLED` to using `GAS_ESTIMATOR_MODE` instead, since support for the old env var may be dropped in a future release.
